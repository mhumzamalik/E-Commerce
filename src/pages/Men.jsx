...
export default function Man() {
  ...
  if (loading) {
    return <p className="text-center py-16 text-gray-500">Loading products...</p>;
  }

  return (
    <section className="px-8 py-16 bg-white text-black">
      <h2 className="text-3xl font-bold mb-8 text-center">MAN COLLECTION</h2>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {products.map((item) => (
          <Link to={`/product-men/${item.id}`} key={item.id}>
